...
class Model4(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 100, kernel_size=(1, 16 * 768), stride=1)  # params: 输入通道数，输出通道数（filter个数），核视野（H,W）,步长
    ...
    def soft_align_attention(self, vec1, vec2):  # 输入vec1，vec2  局部推理
        ...
        vec1_align = torch.bmm(softmax_attention1_2, vec2)
        vec2_align = torch.bmm(softmax_attention2_1, vec1)  # x2_align  b*sq2*400
        return vec1_align, vec2_align  # x2_align  b*sq*(4*768)
    ...
    def fix_encoder(self, vec1, vec2, vec1_align, vec2_align):  # 形成新的字嵌入
        ...
        x1_combined = torch.cat([vec1, vec1_align, x1_sub, x1_mul], dim=2)  # b*sq*（768*12）
    ...
    def forward(self, ids_tensor1, token_type_ids1, attention_mask1, ids_tensor2, token_type_ids2, attention_mask2):
        vec1 = self.bert_encoder(ids_tensor1, token_type_ids1, attention_mask1)  # b*q*(4*768)
        vec2 = self.bert_encoder(ids_tensor2, token_type_ids2, attention_mask2)

        vec1_align, vec2_align = self.soft_align_attention(vec1, vec2)
        x1_combined, x2_combined = self.fix_encoder(vec1, vec2, vec1_align, vec2_align)

        cnn_encoder_vector1 = self.cnn_encoder(x1_combined)
        cnn_encoder_vector2 = self.cnn_encoder(x2_combined)
